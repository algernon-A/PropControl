            ...
            foreach (PluginManager.PluginInfo plugin in PluginManager.instance.GetPluginsInfo())
            {
                foreach (Assembly assembly in plugin.GetAssemblies())
                {
                    switch (assembly.GetName().Name)
                    {
                        ...
                        case "PropSnapping":
                            // Prop Snapping. Okay to just be disabled.
                            if (plugin.isEnabled)
                            {
                                conflictDetected = true;
                                conflictingModNames.Add("Prop Snapping");
                            }

                            break;

                        case "PropPrecision":
                            // Prop Precision. Okay to just be disabled.
                            if (plugin.isEnabled)
                            {
                                conflictDetected = true;
                                conflictingModNames.Add("Prop Precision");
                            }

                            break;

                        case "AdaptivePropVisibilityDistance":
                            // Adaptive Prop Visibility Distance. Okay to just be disabled.
                            if (plugin.isEnabled)
                            {
                                conflictDetected = true;
                                conflictingModNames.Add("Adaptive Prop Visibility Distance");
                            }

                            break;

                        case "DecalFix":
                            // Decal Prop Fix. Okay to just be disabled.
                            if (plugin.isEnabled)
                            {
                                conflictDetected = true;
                                conflictingModNames.Add("Decal Prop Fix");
                            }
